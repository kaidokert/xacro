...
impl IncludeProcessor {
    ...
    pub fn process(
        ...
        IncludeProcessor::process_element(&mut xml, xml_path, xacro_ns)?;
        Ok(xml)
    }
    ...
    fn process_element(
        ...
        for child in &element.children {
            if let NodeElement(mut child_element) = child.clone() {
                if is_xacro_element(&child_element, "include", xacro_ns) {
                    if let Some(filename) = child_element.attributes.get("filename") {
                        ...
                        IncludeProcessor::process_element(
                            &mut included_element,
                            xml_path,
                            xacro_ns,
                        )?;
                    ...
                    IncludeProcessor::process_element(&mut child_element, xml_path, xacro_ns)?;
